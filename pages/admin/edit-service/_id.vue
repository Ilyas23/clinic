<template>
  ...
  >
    ...
    <h1 class="mb">Редактировать услугу</h1>
...
export default {
  head() {
    return {
      title: `Редактировать услугу | ${process.env.appName}`
    }
  },
  layout: 'admin',
    ...
    onSubmit(){
      this.$refs.form.validate(async valid => {
        if(valid){
          try{
            this.loading = true
            await this.$store.dispatch('service/update', this.form)
            this.form.title = ''
            this.form.text = ''
            this.$message.success('Услуга сохранена')
          }catch(e){} finally{
            this.loading = false
          }
